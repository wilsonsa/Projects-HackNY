...
def makeGameInstructionPage():
    if canvas.data.gameMode==2:
        canvas.create_rectangle(0,0, canvas.data.canvasWidth, canvas.data.canvasHeight, fill="Green")
        canvas.create_text(canvas.data.canvasWidth/12,50, text= "GAME INSTRUCTIONS HERE", anchor=NW,fill="Black", font="times 23 underline" )
        canvas.create_text(canvas.data.canvasWidth/12+25,90,
                           text= "Scrabble Tiles\n\n\n\n\n"+
                           "Board Point Values\n\n\n\n\n"+
                           "Starting a Game\n\n\n"+
                           "End of a Game", anchor=NW,fill="Black", font="times 22" )
        canvas.create_text(canvas.data.canvasWidth/12+50,130, text= "-A Scrabble game board is a 15 x 15 square grid."+
                           "Scrabble is played with exactly100 tiles. 98 of these tiles contain\n"
                           "  letters on them, while there are 2 blank tiles. These blank tiles add a wildcard aspect to Scrabble. \n"+
                           "-Various letters have different point values, depending on the rarity of the letter and the difficulty in playing it. Blank\n"+
                           "  tiles have no point value.\n\n\n"+
                           "-Some squares on the Scrabble board represent multipliers. If a tile is placed on this square, then the tile's value is\n"+
                           "  multiplied by a factor or either 2x or 3x. Certain tiles multiply the point value of an entire word and not simply the\n"+
                           "  tile on that space.\n"
                           "-These multipliers only work once.\n\n\n"+
                           "-Each player starts with seven pieces. The player can do one of three things on a turn. The player can place a word,\n"+
                           "  exchange tiles or pass.\n\n\n"+
                           "-When all of the tiles have been taken from the bag and one player has used all of the tiles on their rack, then the \n"+
                           "game ends.\n"+
                           "-The Scrabble player with the highest score after all final scores are tallied wins."
                           , anchor=NW,fill="Black", font="times 18" )
        canvas.create_window(canvas.data.canvasWidth/16, canvas.data.canvasHeight*13/15, window=canvas.data.button4)
        
def makeKeyInstructionPage():
    if canvas.data.gameMode==3:
        canvas.create_rectangle(0,0, canvas.data.canvasWidth, canvas.data.canvasHeight, fill="Green")
        canvas.create_text(5,15, text= "GAME KEY EXPLANATIONS\n",anchor=NW, fill="Black", font="times 23 underline" )
        canvas.create_text(30,70, text="-Tiles, found on the bottom  rack, can be dragged directly onto the board.\n\n"+
                           "\n-On the right, clicking the GRAB button fills empty space in rack with a \n"+
                           "  random letter.\n\n"+
                           "\n-The box on the right records the earned score for each player. At the end\n"+
                           "  of the game, the player with highest score wins.\n\n"+
                           "-The SCRAMBLE button on the right rearranges the letters of the tiles on \n"+
                           "  your rack.\n"
                           "-The SUBMIT button on the right is pressed when you've completed a \n"+
                           "  word on the board. If the word you've placed it valid, you can continue\n"+
                           "  on, otherwise, you must rearrange.\n\n"+
                           "-The box containing the number on the right indicated the number of \n"+
                           "  pieces left in the bag.\n\n"+
                           "-The box containing the 7 blank cells is your Tile Holder. It will \n"+
                           "  contain the tiles that you will drag onto the board"
                           ,anchor=NW, fill="Black", font="times 18" )   
        canvas.create_window(canvas.data.canvasWidth/16, canvas.data.canvasHeight*13/15, window=canvas.data.button5)
...
#These are grabBag Coordinates
        ...
        if (x1<event.x<x2 and y1<event.y<y2): #if click in Grab Box
            ...
            if (canvas.data.gameStatus=="First Move"):
                if wordPlacementIsValid()==True:
                    ...
                    canvas.data.piecesPlacedInATurn=[] #resets pieces placed in turn
                ...
                print "GameStatus:",canvas.data.gameStatus
                #changeTiles()#takes player2 tiles and places over player1 Tiles
                ...
                if wordPlacementIsValid()==True:
                    ...
                    canvas.data.piecesPlacedInATurn=[]
                print "GameStatus:",canvas.data.gameStatus
                #changeTiles()#takes player1 tiles and palces over player2 tiles
                #wordPlacementIsValid()
    #These are placerCell Coordinates
        ...
        #This is when you're actually moving a piece already on the board to another place    
        elif ((boardDimensionX1<event.x<boardDimensionX2 and boardDimensionY1<event.y<boardDimensionY2) and
               isOccupied(viewToModel(event.x, event.y)[0],viewToModel(event.x, event.y)[1])):
            #print isOccupied(viewToModel(event.x, event.y)[0],viewToModel(event.x, event.y)[1])
            canvas.data.pieceInHand=True

            tileInfo=canvas.data.tilesOnBoard[viewToModel(event.x, event.y)[0]][viewToModel(event.x, event.y)[1]]
            #use the thing on the print function that takes in the different x and y
            liftPieceFromBoard(tileInfo)
            #once life piece from board, must remove piece from canvas.data.tilesOnBoard
...
def leftMouseReleased(event):
    if (10<event.x<730 and 10<event.y<730 and canvas.data.pieceInHand==True):  #within the Board Dimensions
        x=event.x
        y=event.y
    ...
    elif((not(10<event.x<730 and 10<event.y<730) #if it's not on board, move back to Rack
         and canvas.data.pieceInHand==True)):
        tileInfo=canvas.data.currentPiece
        moveBackToRack(tileInfo)

        #canvas.data.score=canvas.data.score-computeScore(tileInfo.row, tileInfo.col, tileInfo)
...
def computeScore(row, col, tile):
    if canvas.data.twoDCoordinates[row][col]=="DeepSkyBlue":
        canvas.data.score+=3*tile.value
    elif canvas.data.twoDCoordinates[row][col]=="PaleTurquoise":
        canvas.data.score+=2*tile.value
    else:
        canvas.data.score+=tile.value
...
def isABlank(tile):
    if (tile.letter==" "):
        ...
        while (len(newLetter)!=1 or ((ord(newLetter)<65) or 90<ord(newLetter)<97 or ord(newLetter)>122)):
            if(len(newLetter)!=1):
                newLetter= raw_input("One letter only please: ")        
            elif((ord(newLetter)<65) or 90<ord(newLetter)<97 or ord(newLetter)>122):
    ...
    for row in range (len(board)):
        for col in range (len(board[0])):
            if board[row][col]!=False:
                #canvas.data.currentPiece=board[row][col]
                #canvas.data.occupiedBoardSpaces+=(canvas.data.currentPiece.row, canvas.data.currentPiece.col)
...
def drawTile(piece):
    ...
    letter=piece.letter
    value=piece.value
    canvas.create_text(((x1+x0)/2),((y1+y0)/2),text=letter, fill="Black", font = "times 23", activefill="blue")
    canvas.create_text(((x1+x0)/2+16),((y1+y0)/2+14),text=value, fill="Black", font = "times 10", activefill="blue")
...
def drawPlayerTurn():
    ...
    canvas.create_text(793, 640, text=playerTurn, anchor=NW, fill="Gold", font="Times 20 underline")
def redrawAll():
    canvas.delete(ALL)
    if canvas.data.gameMode==1:
        drawBoard()
...
def drawPiece(x, y, tileInfo):
    ...
    canvas.create_text(((x1+x0)/2),((y1+y0)/2),text=letter, fill="Black", font = "times 20")
    canvas.create_text(((x1+x0)/2+16),((y1+y0)/2+14),text=value, fill="Black", font = "times 10")
...
def drawBoard():
    canvas.create_rectangle(0,0,
                            canvas.data.boardWidth, canvas.data.boardHeight,
                            fill="gold")
    ...
    canvas.create_window(canvas.data.canvasWidth*15/16, canvas.data.canvasHeight*13/15, window=canvas.data.button6)
...
def drawTilePlacer():
    ...
    canvas.data.tilePlacerCoordX1=canvas.data.boardWidth+canvas.data.colWidth+(canvas.data.colWidth*7)+canvas.data.margin1*2 
    canvas.data.tilePlacerCoordY1=canvas.data.canvasHeight-100+canvas.data.margin1*2
...
def drawTilePlacerCell(row,col):
    ...
    x0 = canvas.data.margin1 + col * canvas.data.colWidth+canvas.data.tilePlacerCoordX0
    
    y0 = canvas.data.margin1 + row * canvas.data.rowHeight+canvas.data.tilePlacerCoordY0
...
def drawCell(row,col):
    ...
    #print "TopLeft:",x0, ",", y0, "          ","BotRight:",x1,",",y1 #DIMENSIONS OF EACH CELL
...
def drawCenterStar():    
    verts = [5,20,20,20,25,5,30,20,45,20,32.5,30,37.5,45,25,35,12.5,45,17.5,30]
...
def writeBoardWords(row,col):
    ...
    #print canvas.data.twoDCoordinates
    """
    if (canvas.data.twoDCoordinates[row][col]=="Red" and threeTimesWS(row,col)):
        written="Triple\nWord\nScore"
        canvas.create_text((x0+x1)/2, (y0+y1)/2, text=written, fill="black", font="NewsGothic 7 bold")
    if (canvas.data.twoDCoordinates[row][col]=="IndianRed1" and twoTimesWS(row,col)):
        written="Double\nWord\nScore"
        canvas.create_text((x0+x1)/2, (y0+y1)/2, text=written, fill="black", font="NewsGothic 7 bold")
    if (canvas.data.twoDCoordinates[row][col]=="IndianRed1" and centerPiece(row,col)):
        drawCenterStar()
    if (canvas.data.twoDCoordinates[row][col]=="DeepSkyBlue"):
        written="Triple\nLetter\nScore"
        canvas.create_text((x0+x1)/2, (y0+y1)/2, text=written, fill="black", font="NewsGothic 7 bold")
    if (canvas.data.twoDCoordinates[row][col]=="PaleTurquoise"):
        written="Double\nLetter\nScore"
        canvas.create_text((x0+x1)/2, (y0+y1)/2, text=written, fill="black", font="NewsGothic 7 bold")
    """
    if (threeTimesWS(row,col)):
...
def drawSideBoard():
    canvas.create_rectangle(canvas.data.boardWidth, 0,canvas.data.canvasWidth,canvas.data.canvasHeight,
                            fill="royalblue")
...
def drawGrabBag():
    ...
    canvas.create_rectangle(x1,y1,x2,y2,
                            fill="brown")
    canvas.create_text((x1+x2)/2,(y1+y2)/2, text="Grab!", fill="Gold", font="Helvetica 40")
...
def drawScoreBoard():
    ...
    canvas.create_rectangle(x1,y1,x2,y2,
                            fill="brown")
    score1=canvas.data.score
    score2=0
    canvas.create_text(((x2-x1)/4)+x1,(y1+y2)/2-25, text="Player1 Score:", fill="Gold", font="Helvetica 15")
    canvas.create_text(((x2-x1)/4)+x1,(y1+y2)/2+20, text=score1, fill="Gold", font="Helvetica 15")
    
    canvas.create_text(((x2-x1)*3/4)+x1,(y1+y2)/2-25, text="Player2 Score:", fill="Gold", font="Helvetica 15")
    canvas.create_text(((x2-x1)*3/4)+x1,(y1+y2)/2+20, text=score2, fill="Gold", font="Helvetica 15")
...
def drawRackScrambler():
    ...
    canvas.create_text(x1+((x2-x1)/2), y1+((y2-y1)/2), text= "Scramble", fill = "Gold", font = "Helvetica 12" )
...
def drawSubmitButton():
    ...
    canvas.create_rectangle(x1, y1, x2, y2, fill="brown")
    canvas.create_text(x1+((x2-x1)/2), y1+((y2-y1)/2), text= "Submit", fill = "Gold", font = "Helvetica 12" )
...
def drawNumPiecesLeft():
    ...
    canvas.create_rectangle(x1, y1, x2, y2, fill="brown")
    canvas.create_text(x1+((x2-x1)/2), y1+((y2-y1)/2), text= piecesLeft, fill = "Gold", font = "Helvetica 12" )
...
def makePiece(): #takes a random piece and places it on rack
    ...
    if len(canvas.data.piecesOnRack)<7:#number of Pieces allowed
        ...
        newPiece= Tile(piece, pointValue, -1, col) #row and col are currently "Dummy Values"... aka Noting it's Off the Board
        #print newPiece.col
...
def storePiecesOnRack(newPiece):
    c=0
    canvas.data.piecesOnRack.append(newPiece)   #canvas.data.piecesOnRack containes list of tiles
    #print "*************", canvas.data.piecesOnRack[0]
    for row in range (len(canvas.data.piecesOnRack)):
        drawPieces(canvas.data.piecesOnRack[row],c)
        canvas.data.coordsOnRack+=1
...
def drawTileIntoPlacerCell(row,col, piece, pointValue):
    ...
    #print "TopLeft:",x0, ",", y0, "          ","BotRight:",x1,",",y1
    canvas.create_rectangle(x0, y0, x1, y1, fill = "LightGoldenrod1", activewidth=3)
    letter=piece
    value=pointValue
    canvas.create_text(((x1+x0)/2),((y1+y0)/2),text=letter, fill="Black", font = "times 20")
    canvas.create_text(((x1+x0)/2+16),((y1+y0)/2+14),text=value, fill="Black", font = "times 10")
...
def pieceLocatedCenter():   
    ...
    for tiles in range(len(canvas.data.piecesPlacedInATurn)):
        if (canvas.data.piecesPlacedInATurn[tiles].row==7 and
            ...
            counter+=1
    if counter == 1:
        return True #need to make so if true, Locks it in place and its no longer your moce
    return False    

def wordPlacementIsValid():
    lastTilePlaced= canvas.data.piecesPlacedInATurn[len(canvas.data.piecesPlacedInATurn)-1]
    if canvas.data.gameStatus=="First Move":
        print "sadhiuay u2y3iue hdsfindsjdhq3u48989hscsa9d82398rn89ndiuweddPOOOOOOOOOOOOOOOP"
       
        print "pieceLocatedCenter Returns:", pieceLocatedCenter()
        print "fitsCrossWord Returns:",fitsCrossWord(lastTilePlaced)
        return pieceLocatedCenter() and fitsCrossWord(lastTilePlaced)
     #### least one letter on the center square (7,7)
    elif canvas.data.gameStatus=="NormalMove1" or canvas.data.gameStatus=="NormalMove2":
...
def analyzeHorizOfVertical(topMost):
    ...
    if len(string)==1:
        ...
        return isAWord(string)  #if use isAWord function, must set nuance that 1 letter word passes
...
def init():
    #print "adfasfdfasfdfasfdfasfdfasfdfasfdfasfdfasfdfasfdfasfdfasfdfasfdfasfdfasfdfasfd"
    ...
    canvas.data.tilePlacerCoordX0=canvas.data.boardWidth+canvas.data.colWidth #tilePlacer Dimensions

    canvas.data.tilePlacerCoordY0=canvas.data.canvasHeight-148
    canvas.data.tilePlacerCoordX1=canvas.data.boardWidth+canvas.data.colWidth+(canvas.data.colWidth*7) 
